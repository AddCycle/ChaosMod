...
public class EntityBear extends EntityTameable {
	...
	private static final DataParameter<Integer> SIT_POSE = EntityDataManager.<Integer>createKey(EntityBear.class,
			DataSerializers.VARINT);
...
//        	|| (stack.getItem() instanceof ItemFishFood);
	...
	@Nullable
	protected ResourceLocation getLootTable() { return LootTableList.ENTITIES_POLAR_BEAR; }
...
//                    this.setTameSkin(1 + this.world.rand.nextInt(3));
	...
	public boolean isStanding() { return ((Boolean) this.dataManager.get(IS_STANDING)).booleanValue(); }
	...
	public int getSitPose() { return this.dataManager.get(SIT_POSE); }
		...
		public boolean shouldExecute() {
			if (EntityBear.this.isChild()) {
				...
				if (super.shouldExecute()) {
					for (EntityBear entitypolarbear : EntityBear.this.world.getEntitiesWithinAABB(EntityBear.class,
							EntityBear.this.getEntityBoundingBox().grow(8.0D, 4.0D, 8.0D))) {
	...
	public static enum EnumVariant {
		...
		// TODO : if more variants are added, needs to change otherwise might cause
		// issues
