...
import net.chaos.chaosmod.network.packets.PacketLaunchFinalCredits;
...
public class ChaosMasterBoss extends EntityLiving implements IMob, IEntityMultiPart {
	public static final DataParameter<Integer> PHASE = EntityDataManager.<Integer>createKey(ChaosMasterBoss.class,
	...
	private final CMFightManager fightManager;
	/** The dragon phase manager */
	private final CMPhaseManager phaseManager;
	...
	public ChaosMasterBoss(World worldIn) {
		super(worldIn);
		...
		this.phaseManager = new CMPhaseManager(this);
	}
	...
	protected void entityInit() {
		super.entityInit();
		this.getDataManager().register(PHASE, Integer.valueOf(CMPhaseList.HOVER.getId()));
	}
			...
			if (this.isAIDisabled()) {
				...
				if (this.world.isRemote) {
					...
					if (vec3d != null) {
						...
						if (this.slowed) {
							this.move(MoverType.SELF, this.motionX * 0.800000011920929D,
				...
				if (!this.world.isRemote && this.hurtTime == 0) {
					this.collideWithEntities(this.world.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartWing1
							.getEntityBoundingBox().grow(4.0D, 2.0D, 4.0D).offset(0.0D, -2.0D, 0.0D)));
					...
					this.attackEntitiesInList(this.world.getEntitiesWithinAABBExcludingEntity(this,
							this.dragonPartHead.getEntityBoundingBox().grow(1.0D)));
					this.attackEntitiesInList(this.world.getEntitiesWithinAABBExcludingEntity(this,
							this.dragonPartNeck.getEntityBoundingBox().grow(1.0D)));
				}
	...
	private void updateDragonEnderCrystal() {
		if (this.healingEnderCrystal != null) {
			if (this.healingEnderCrystal.isDead) {
				...
				this.setHealth(this.getHealth() + 2.0F); // healing
			}
		}
					...
					if (!block.isAir(iblockstate, this.world, blockpos) && iblockstate.getMaterial() != Material.FIRE) {
						if (!net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.world, this)) {
							flag = true;
								...
								&& this.canDestroySpecificBlock(iblockstate.getBlock())
	...
	private boolean canDestroySpecificBlock(Block block) {
		return block != Blocks.BARRIER && block != Blocks.OBSIDIAN && block != Blocks.END_STONE
				&& block != Blocks.BEDROCK && block != Blocks.END_PORTAL && block != Blocks.END_PORTAL_FRAME
				&& block != Blocks.COMMAND_BLOCK && block != Blocks.REPEATING_COMMAND_BLOCK
				&& block != Blocks.CHAIN_COMMAND_BLOCK && block != Blocks.IRON_BARS && block != Blocks.END_GATEWAY;
	}
		...
		if (damage < 0.01F) {
			...
			if (source.getTrueSource() instanceof EntityPlayer || source.isExplosion()) {
				...
				if (this.getHealth() <= 0.0F && !this.phaseManager.getCurrentPhase().getIsStationary()) {
					this.setHealth(1.0F);
					this.phaseManager.setPhase(CMPhaseList.DYING);
				}
				...
				if (this.phaseManager.getCurrentPhase().getIsStationary()) {
					...
					if ((float) this.sittingDamageReceived > 0.25F * this.getMaxHealth()) {
						...
						this.phaseManager.setPhase(CMPhaseList.TAKEOFF);
					}
				}
			}
	...
	protected void onDeathUpdate() {
		...
		if (this.deathTicks == 200 && !this.world.isRemote) {
			...
			// Display final credits to all players
			PacketManager.network.sendToAll(new PacketLaunchFinalCredits());
		}
	}
	...
	public Path findPath(int startIdx, int finishIdx, @Nullable PathPoint andThen) {
		...
		if (pathpoint1 == pathpoint4) {
			...
			// LOGGER.debug("Failed to find path from {} to {}", Integer.valueOf(startIdx),
			// Integer.valueOf(finishIdx));
	...
	public static void registerFixesDragon(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, ChaosMasterBoss.class);
	}
	...
	public void notifyDataManagerChange(DataParameter<?> key) {
		if (PHASE.equals(key) && this.world.isRemote) {
			this.phaseManager.setPhase(CMPhaseList.getById(((Integer) this.getDataManager().get(PHASE)).intValue()));
		}

		super.notifyDataManagerChange(key);
	}

	public CMPhaseManager getPhaseManager() {
		return this.phaseManager;
	}

	@Nullable
	public CMFightManager getFightManager() {
