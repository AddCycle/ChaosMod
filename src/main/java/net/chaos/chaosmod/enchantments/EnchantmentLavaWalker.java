...
public class EnchantmentLavaWalker extends AbstractEnchantment {
	...
	public static Block getCompatibleMagmaBlock() {
			...
			public void onEntityWalk(World worldIn, BlockPos pos, Entity entityIn) {
				if (!entityIn.isImmuneToFire() && entityIn instanceof EntityLivingBase
					...
					if (entityIn instanceof EntityPlayer) {
						...
						boolean condition = !boots.isEmpty() && boots.isItemEnchanted()
								&& EnchantmentHelper.getEnchantmentLevel(ModEnchants.LAVA_WALKER, boots) > 0;
						if (!condition) {
		...
		}).setHardness(0.5F).setSoundType(SoundType.STONE).setUnlocalizedName("magma").setRegistryName("minecraft", "magma");
	}
}