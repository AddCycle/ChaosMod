...
public class CustomLeaves extends BlockLeaves implements IBlockModel {
	public static final PropertyEnum<CustomLeafVariant> VARIANT = PropertyEnum.create("variant",
			CustomLeafVariant.class);
	...
	public CustomLeaves(String name) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, CustomLeafVariant.SNOWY)
				.withProperty(DECAYABLE, true).withProperty(CHECK_DECAY, false));
	...
	protected void dropApple(World worldIn, BlockPos pos, IBlockState state, int chance) {
		int fortune = EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE,
				state.getBlock().getDefaultState().getBlock().getItem(worldIn, pos, state));
		int base = 20;
		...
		} else if (worldIn.rand.nextInt(chance) == 0) {
	...
	public enum CustomLeafVariant implements IStringSerializable {
		SNOWY(0, "snowy"), MAPLE(1, "maple"), ENDER(2, "ender"), OLIVE(3, "olive");
