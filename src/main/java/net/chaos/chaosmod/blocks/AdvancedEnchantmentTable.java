...
import net.chaos.chaosmod.tileentity.TileEntityAdvancedEnchantmentTable;
...
public class AdvancedEnchantmentTable extends BlockContainerBase {
	protected static final AxisAlignedBB AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.75D, 1.0D);

	public AdvancedEnchantmentTable() {
        super("advanced_enchantment_table", Material.ROCK, MapColor.RED);
			...
			for (int j = -2; j <= 2; ++j) {
				...
				if (rand.nextInt(16) == 0) {
					for (int k = 0; k <= 1; ++k) {
						...
						if (ForgeHooks.getEnchantPower(worldIn, blockpos) > 0) {
