...
public class WorldGenChaosLandPodium extends WorldGenerator {
	...
	public WorldGenChaosLandPodium(boolean activePortalIn)
	{
	...
	public boolean generate(World worldIn, Random rand, BlockPos position)
	{
		for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(new BlockPos(position.getX() - 4, position.getY() - 1, position.getZ() - 4), new BlockPos(position.getX() + 4, position.getY() + 32, position.getZ() + 4)))
		{
			...
			if (d0 <= 3.5D)
			{
				...
				{
					this.setBlockAndNotifyAdequately(worldIn, new BlockPos(blockpos$mutableblockpos), ModBlocks.CHAOSLAND_PORTAL.getDefaultState());
				}
				else
				{
