...
public class MapGenCustomCaves extends MapGenBase {
	...
	protected void addRoom(long seed, int chunkX, int chunkZ, ChunkPrimer primer, double originX, double originY,
			double originZ) {
		this.addTunnel(seed, chunkX, chunkZ, primer, originX, originY, originZ, 1.0F + this.rand.nextFloat() * 6.0F,
				0.0F, 0.0F, -1, -1, 0.5D);
	}

	protected void addHugeRoom(long seed, int chunkX, int chunkZ, ChunkPrimer primer, double originX, double originY,
			double originZ) {
	 ...
	 */
	protected void addTunnel(long seed, int chunkX, int chunkZ, ChunkPrimer primer, double originX, double originY,
			double originZ, float radius, float yawAngle, float pitchAngle, int currentStep, int totalSteps,
		...
		for (boolean flag = random.nextInt(6) == 0; currentStep < totalSteps; ++currentStep) {
			double d2 = 1.5D
					+ (double) (MathHelper.sin((float) currentStep * (float) Math.PI / (float) totalSteps) * radius);
			...
			if (!flag2 && currentStep == j && radius > 1.0F && totalSteps > 0) {
				this.addTunnel(random.nextLong(), chunkX, chunkZ, primer, originX, originY, originZ,
						random.nextFloat() * 0.5F + 0.5F, yawAngle - ((float) Math.PI / 2F), pitchAngle / 3.0F,
						currentStep, totalSteps, 1.0D);
				this.addTunnel(random.nextLong(), chunkX, chunkZ, primer, originX, originY, originZ,
						random.nextFloat() * 0.5F + 0.5F, yawAngle + ((float) Math.PI / 2F), pitchAngle / 3.0F,
						currentStep, totalSteps, 1.0D);
				return;
			...
			if (flag2 || random.nextInt(4) != 0) {
				double d4 = originX - chunkCenterX;
				double d5 = originZ - chunkCenterZ;
				...
				if (originX >= chunkCenterX - 16.0D - d2 * 2.0D && originZ >= chunkCenterZ - 16.0D - d2 * 2.0D
						&& originX <= chunkCenterX + 16.0D + d2 * 2.0D && originZ <= chunkCenterZ + 16.0D + d2 * 2.0D) {
					int k2 = MathHelper.floor(originX - d2) - chunkX * 16 - 1;
					int k = MathHelper.floor(originX + d2) - chunkX * 16 + 1;
					int l2 = MathHelper.floor(originY - d3) - 1;
					int l = MathHelper.floor(originY + d3) + 1;
					int i3 = MathHelper.floor(originZ - d2) - chunkZ * 16 - 1;
					int i1 = MathHelper.floor(originZ + d2) - chunkZ * 16 + 1;
					...
					if (!flag3) {
						...
						for (int j3 = k2; j3 < k; ++j3) {
							double d10 = ((double) (j3 + chunkX * 16) + 0.5D - originX) / d2;

							for (int i2 = i3; i2 < i1; ++i2) {
								double d8 = ((double) (i2 + chunkZ * 16) + 0.5D - originZ) / d2;
								boolean flag1 = false;

								if (d10 * d10 + d8 * d8 < 1.0D) {
									for (int j2 = l; j2 > l2; --j2) {
										double d9 = ((double) (j2 - 1) + 0.5D - originY) / d3;
										...
										if (d9 > -0.7D && d10 * d10 + d9 * d9 + d8 * d8 < 1.0D) {
											...
											digBlock(primer, j3, j2, i2, chunkX, chunkZ, flag1, iblockstate1,
													iblockstate2);
										}
									}
								}
