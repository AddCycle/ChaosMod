...
public class WorldGenBirchTreeHive extends WorldGenAbstractTree {
    ...
    public WorldGenBirchTreeHive(boolean notify, boolean useExtraRandomHeightIn)
    {
        super(notify);
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
                    ...
                    BlockPos hivePos = hiveBase.offset(facing);

                    if (worldIn.isAirBlock(hivePos)) {
                    	worldIn.setBlockState(hivePos, ModBlocks.BEEHIVE.getDefaultState()
